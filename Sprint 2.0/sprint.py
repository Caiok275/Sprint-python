...
def cadastrar():
    try:
        ...
        sql = """ INSERT INTO T_HCFMUSP_CONSULTA (id_paciente,id_doutor,dt_consulta)VALUES (:1,:2,:3)"""
        inst_cadastro.execute(sql,(id_paciente,id_doutor,dt_consulta))
        conn.commit()
...
def listar_doutores(sql: str) -> str:  
...
# funcao que lista todos os itens da tabela
def listar_dados_consulta(sql: str, parametro:str = None) -> str:  
...
def gerar_arquivo():
    ...
    df = listar_dados_consulta("SELECT * FROM T_PRODUTO")
    try:
