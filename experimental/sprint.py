...
def gravar_json(nome_json,consulta):
    # Verifica se o arquivo existe e tenta carregar os dados existentes
    ...
    # Grava todos os dados de volta no arquivo
    with open(nome_json, 'w', encoding='utf-8') as f:
        json.dump(dados, f, ensure_ascii=False, indent=4)

def ler_json(arq_agenda):
        if os.path.exists(arq_agenda):
            with open(arq_agenda, 'r', encoding='utf-8') as f:
                try:
                    dados = json.load(f)
                    if not isinstance(dados, list):
                        dados = []
...
def mostrar_consultas(arq_agenda):
    consultas = ler_json(arq_agenda)
    ...
    for i, consulta in enumerate(consultas, start=1):
        print(f"\nConsulta {i}:")
        print(f"  Usuário        : {consulta.get('Usuário')}")
        print(f"  Tipo de consulta: {consulta.get('Tipo de consulta')}")
        print(f"  Doutor         : {consulta.get('Doutor')}")
        print(f"  Hora           : {consulta.get('Hora')}")
# ================= Menu Principal =================
def menu():
